...
public class Comman {
	...
	public static final String getAllRequestStatusList(){
		return "SELECT r.id, ud.organization_name, u.full_name, r.blood_group, r.unit, CASE WHEN r.status = 1 THEN 'Approved' WHEN r.status = 2 THEN 'Rejected' ELSE 'Pending' END AS status, CASE WHEN r.user_type = 1 THEN 'Donar' ELSE 'Seeker' END AS user_type, DATE(r.created_on) AS created_on FROM user_details u INNER JOIN user_request r ON u.user_id = r.user_id INNER JOIN user_details ud ON r.to_blood_bank = ud.user_id;";
	}
